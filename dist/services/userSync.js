...
exports.getOrSyncUser = getOrSyncUser;
exports.getUserByEmail = getUserByEmail;
exports.updateUser = updateUser;
exports.formatUserForAPI = formatUserForAPI;
 ...
 */
async function syncUserFromClerk(clerkUserId) {
    ...
    try {
        ...
        const clerkUser = await clerk_1.clerkClientInstance.users.getUser(clerkUserId);
        if (!clerkUser) {
        ...
        const user = await users_1.default.findOneAndUpdate({ clerk_id: clerkUserId }, {
            $set: {
                ...userData,
                updated_at: new Date(),
            },
            $setOnInsert: {
                ...
                verification_status: 'unverified', // New users start as unverified
            },
        }, {
...
async function getOrSyncUser(clerkUserId) {
    ...
    if (!(0, database_1.isDatabaseConnected)()) {
        ...
        console.error(`   Connection state: ${mongoose_1.default.connection.readyState}`);
    ...
    try {
        ...
        let user = await users_1.default.findOne({ clerk_id: clerkUserId });
...
async function getUserByEmail(email) {
    ...
    try {
        const user = await users_1.default.findOne({ email: email.toLowerCase().trim() });
        return user;
    }
    catch (error) {
 ...
 */
async function updateUser(clerkUserId, updateData) {
    // Check if database is connected before making queries
    if (!(0, database_1.isDatabaseConnected)()) {
    ...
    try {
        ...
        if (!user) {
            ...
            try {
                ...
                return await users_1.default.findOneAndUpdate({ clerk_id: clerkUserId }, {
                    $set: {
                        ...updateData,
                        updated_at: new Date(),
                    },
                ...
                const existingUser = await users_1.default.findOne({ clerk_id: clerkUserId });
                if (existingUser) {
