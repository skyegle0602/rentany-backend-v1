...
router.get('/', async (req, res) => {
    try {
        ...
        const viewedItems = await viewedItems_1.default.find(query).sort({ viewed_date: -1 });
...
router.post('/', clerk_1.requireAuth, async (req, res) => {
    try {
        ...
        const viewedItem = await viewedItems_1.default.findOneAndUpdate({ user_email, item_id }, {
        ...
        if (error.code === 11000) {
            ...
            try {
                const existing = await viewedItems_1.default.findOne({
...
router.put('/:id', clerk_1.requireAuth, async (req, res) => {
    try {
        ...
        const viewedItem = await viewedItems_1.default.findById(id);
        if (!viewedItem) {
...
router.delete('/:id', clerk_1.requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const viewedItem = await viewedItems_1.default.findByIdAndDelete(id);
        if (!viewedItem) {
            return res.status(404).json({
                success: false,
                error: 'Viewed item not found',
