...
const disputes_1 = __importDefault(require("../models/disputes"));
...
router.get('/', clerk_1.requireAuth, async (req, res) => {
    try {
        ...
        const currentUser = await users_1.default.findOne({ clerk_id: userId });
        if (!currentUser) {
        ...
        // Fetch disputes
        const disputes = await disputes_1.default.find(query)
...
router.post('/', clerk_1.requireAuth, async (req, res) => {
    try {
        ...
        const { rental_request_id, filed_by_email, against_email, reason, description, evidence_urls, } = req.body;
        // Basic validation
        ...
        // Create new dispute
        const newDispute = new disputes_1.default({
            rental_request_id,
            filed_by_email,
            against_email,
            reason,
...
router.put('/:id', clerk_1.requireAuth, async (req, res) => {
    try {
        ...
        // Find the dispute
        const dispute = await disputes_1.default.findById(id);
        if (!dispute) {
