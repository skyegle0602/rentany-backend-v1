 ...
 */
router.post('/diagnostic', clerk_1.requireAuth, async (req, res) => {
    try {
        const result = {
            success: false,
        };
        ...
        const apiKey = env_1.STRIPE_SECRET_KEY;
...
router.post('/webhook', async (req, res) => {
    ...
    try {
        ...
        if (event.type.startsWith('identity.verification_session.')) {
            ...
            try {
                const updatedUser = await (0, userSync_1.updateUser)(userId, {
                ...
                if (updateError.status === 404 && updateError.clerkError) {
                    ...
                    try {
                        const User = (await Promise.resolve().then(() => __importStar(require('../models/users')))).default;
                        const directUpdate = await User.findOneAndUpdate({ clerk_id: userId }, {
                            $set: {
                                verification_status: verificationStatus,
                                updated_at: new Date(),
                            },
 ...
 */
router.post('/connect/onboarding', clerk_1.requireAuth, async (req, res) => {
    try {
        if (!stripe) {
            return res.status(500).json({
                success: false,
 ...
 */
router.get('/connect/status', clerk_1.requireAuth, async (req, res) => {
    try {
        if (!stripe) {
            return res.status(500).json({
                success: false,
 ...
 */
router.post('/connect/payout', clerk_1.requireAuth, async (req, res) => {
    try {
        if (!stripe) {
            return res.status(500).json({
                success: false,
