...
import { CLERK_SECRET_KEY, CLERK_PUBLISHABLE_KEY } from '../config/env'
...
export const clerkClientInstance = createClerkClient({
...
export const publicRoutes = (
  ...
  const isPublicPath = publicPaths.some(publicPath => 
    fullPath.startsWith(publicPath) || relativePath.startsWith(publicPath.replace('/api/', '/'))
  )
...
export async function getCurrentUser(req: Request) {
  ...
  try {
    ...
    const toISOString = (timestamp: number | Date | undefined): string | undefined => {
    ...
    return {
      ...
      verification_status: user.emailAddresses[0]?.verification?.status === 'verified' 
        ? 'verified' as const 
        : 'unverified' as const,
      created_at: toISOString(user.createdAt as number | Date | undefined),
      updated_at: toISOString(user.updatedAt as number | Date | undefined),
    }
  } catch (error) {
